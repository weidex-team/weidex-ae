...
describe('WeiDex Contract', () => {
  ...
  it('should deposit erc20 tokens', async () => {
    ...
    assert(balanceAfter === balanceBefore + amount, 'balance should be increased with the value of deposit');
  })
  ...
  it('should withdraw erc20 tokens', async () => {
    ...
    const balanceAfter = await getBalanceOf(alice, user, token);
    assert(balanceAfter === balanceBefore - amount, 'balance should be decreased with the value of withdraw');
  })
...
async function deployContract(owner, path) {
  ...
  const compiled = await owner.contractCompile(source, { // Compile it
    gas: config.gas
  })
  const instance = await compiled.deploy({ // Deploy it
    options: {
      ttl: config.ttl,
    },
    abi: "sophia"
...
async function mint(owner, address, amount) {
  const minted = await owner.contractCall(erc20.compiled.bytecode, 'sophia', erc20.instance.address, "mint", {
    args: `(${address},${amount})`,
    options: {
      ttl: config.ttl
    },
...
async function deposit(user, token, amount, beneficiary, referral, ae) {
  const deposited = await user.contractCall(weidex.compiled.bytecode, 'sophia', weidex.instance.address, "deposit", {
...
async function withdraw(user, token, amount) {
  const deposited = await user.contractCall(weidex.compiled.bytecode, 'sophia', weidex.instance.address, "withdraw", {
    args: `(${token},${amount})`,
    options: {
      ttl: config.ttl
    },
...
async function getBalanceOf(caller, user, token) {
  const balance = await caller.contractCall(weidex.compiled.bytecode, 'sophia', weidex.instance.address, "balanceOf", {
    args: `(${user},${token})`,
    options: {
      ttl: config.ttl
    },
